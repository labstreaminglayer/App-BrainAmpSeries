...
void MainWindow::read_thread(int deviceNumber, ULONG serialNumber, int impedanceMode, int resolution, int dcCoupling, int chunkSize, int channelCount, std::vector<std::string> channelLabels) {
	...
	std::vector<int16_t> recv_buffer(chunk_words*downsampling_factor, 0);
	...
	for (int i = 0; i < channelCount; i++)
		downsamplers.push_back(new Downsampler<T>(downsampling_factor, chunkSize));
	downsamplers.push_back(new Downsampler<T>(downsampling_factor, chunkSize)); // one extra for the triggers
...
//						for (int c = 0; c < channelCount; c++)
					...
					{
						downsamplers[c]->Downsample(recv_buffer[c + s * (channelCount + 1)]);
						send_buffer[s][c] = scale * downsamplers[c]->m_ptDataOut[s];//recv_buffer[c + s * (channelCount + 1)];
					}
					// buffer for handling triggers
					//trigger_buffer[s] = recv_buffer[channelCount + s*(channelCount+1)];//???
					for (int d = 0; d < downsampling_factor; d++)
					{
						mrkr = recv_buffer[channelCount + (s * downsampling_factor + d) * (channelCount + 1)];
						mrkr ^= g_pull_dir;
