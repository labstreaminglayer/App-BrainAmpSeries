...
#define IOCTL_BA_SETUP CTL_CODE(FILE_DEVICE_UNKNOWN, 0x801, METHOD_BUFFERED, FILE_WRITE_DATA)

// Start acquisition, Parameter [IN]: long nType, 0 = Impedance check, 1 = Data aquisition, 2 =
// calibration
#define IOCTL_BA_START CTL_CODE(FILE_DEVICE_UNKNOWN, 0x802, METHOD_BUFFERED, FILE_WRITE_DATA)

// Stop acquisition
#define IOCTL_BA_STOP CTL_CODE(FILE_DEVICE_UNKNOWN, 0x803, METHOD_BUFFERED, FILE_WRITE_DATA)

// Get buffer filling state, Parameter [OUT]: long nState, < 0 = Overflow, 0 - 100 = Filling state
// in percent.
#define IOCTL_BA_BUFFERFILLING_STATE                                                               \
	CTL_CODE(FILE_DEVICE_UNKNOWN, 0x804, METHOD_BUFFERED, FILE_READ_DATA)

// Get battery voltage of one unit, Parameter [IN]: USHORT nUnit, Parameter [OUT]: long nVoltage in
// millivolts
...
// Set impedance test mode, i.e. even in recording mode, the amplifier is still in impedance check
// mode Parameter [IN]: long bTestMode, != 0 testmode, 0 normal mode
...
//									 2 = 100 kOhm Reference (IMP2 | CAL), 3 = 10 kOhm Reference
//(IMP2) 									 4 = Ground (IMP2)
...
//										Loword:  (error code):	0 = no error, 1 = loss lock, 2 = low
//power, 																3 = can't establish communication at start. 																4 = synchronisation error
#define IOCTL_BA_ERROR_STATE CTL_CODE(FILE_DEVICE_UNKNOWN, 0x809, METHOD_BUFFERED, FILE_READ_DATA)
...
#define IOCTL_BA_DRIVERVERSION CTL_CODE(FILE_DEVICE_UNKNOWN, 0x80E, METHOD_BUFFERED, FILE_READ_DATA)

// Send Commands during recording
// Parameter [IN]: USHORT nCommand (1 = DC Correction), USHORT nChannel (-1 = all channels)
#define IOCTL_BA_COMMAND CTL_CODE(FILE_DEVICE_UNKNOWN, 0x80F, METHOD_BUFFERED, FILE_WRITE_DATA)
...
#define IOCTL_BA_PRESTART CTL_CODE(FILE_DEVICE_UNKNOWN, 0x81a, METHOD_BUFFERED, FILE_WRITE_DATA)

// Poststart, used only if more than device is involved.
#define IOCTL_BA_POSTSTART CTL_CODE(FILE_DEVICE_UNKNOWN, 0x81b, METHOD_BUFFERED, FILE_WRITE_DATA)
